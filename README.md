...
You get basic api description that contains routes and methods, out of the box. You can integrate detailed documentation if you implement the required type class instances.
A sample app with autogenerated documentation is below.
...
Here we have added an endpoint that outputs the description of the api as a list of `RouteInfoSimple` values.
...
The handler function is implemented using `toSimpleRouteInfo` and `toApiDocList` function exported by `Yaar`, as follows.
...
If you access this endpoint using `curl`, you will get something like this..
